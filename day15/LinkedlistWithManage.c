...
void addNodeToList_r(Manage *MList,struct node *new)
{
	if(MList->listNum != 0) //头插
...
void deleteNodeFromManageList(Manage *MList, Elemtype x)
{
	struct node *p=MList->first;
	struct node *pre = p;
	while(p)
...
{   
    ...
    //deleteNodeFromManageList(MList,3);
    // first 就指向了 单链表
	kToManageSortList(MList,1);
    printList(MList);
